...
int string_lcs(struct sysCPS *headRef1, struct sysCPS *headRef2) {
    ...
    if((!strcmp(headRef1->call1, headRef2->call1)) && (!strcmp(headRef1->call2, headRef2->call2))) {
	tmpHead = (struct sysCPS *)malloc(sizeof(struct sysCPS *));
	tmpHead->call1 = strdup(headRef1->call1);
	tmpHead->call2 = strdup(headRef1->call2);
	// if(retHead == NULL) {
	tmpHead->next = retHead;
	retHead = tmpHead;
	// }
        t1 = length_lcs(headRef1->next, headRef2->next);
        val[l1][l2] = (val[l1][l2] > (1 + t1))?val[l1][l2]:(1+t1);
